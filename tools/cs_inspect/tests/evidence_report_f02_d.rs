//! Evidence-report harness for task F02-D (`docs/contracts/CLI-EVIDENCE.md`,
//! schema `schemas/evidence.schema.json`).
//!
//! This test is deliberately **not** named `accept_f02_d_*`: it is not part of
...
//!    `docs/findings/evidence/F02-D.json`.
//!
...
//! shipped `cs-inspect` binary's own `audit` output (the consumer trace),
...
fn evidence_report_f02_d_writes_the_acceptance_report() {
    ...
    assert!(
        ...
        "no `accept_f02_d_` tests were recorded in {}",
        log_path.display()
    );
    ...
    // the spec's required capability for F02-D is exactly that.
    let retail_line = suite
        .assertions
        .iter()
        .find(|(name, _)| name.starts_with("accept_f02_d_retail_"))
        .unwrap_or_else(|| {
            panic!(
                "the retail acceptance test did not run: F02-D requires capability `retail`, \
    ...
    assert!(
        suite
            .assertions
            .iter()
            .any(|(name, _)| name.starts_with("accept_f02_d_")
                && !name.starts_with("accept_f02_d_retail_")),
    ...
    // The consumer trace: the shipped binary's own `audit` output over the
    // original installation under `--scope all --strict`, written into the
    // evidence directory.
    let report_path = evidence_dir.join("audit-report.json");
    ...
    assert!(
        ...
        "cs-inspect audit over $CS_GAME_DIR must succeed for the consumer trace: {}",
        String::from_utf8_lossy(&output.stderr)
    );
    let report = fs::read_to_string(&report_path).expect("the audit report reads back");
    assert!(
    ...
    assert!(
        report.contains("\"full_content\": true"),
        "the retail installation passes strict full-content readiness"
    );
    assert!(
        report.contains("\"unknown\": 0"),
        "the audit leaves zero unclassified gameplay files"
    );
    ...
    let report = format!(
        ...
        jstr(
            "acceptance suite run locally with the retail capability; this harness derives \
              ...
              cs-inspect audit output, rustc and Cargo.lock; validated with \
...
/// root, so a path like `private/evidence/F02-D` written relative to the
...
/// `accept_f02_d_` tests from a recorded `cargo test` output.
fn parse_suite(log: &str) -> Suite {
    ...
    for line in log.lines() {
        ...
        while let Some(position) = cursor.find("test ") {
            ...
            cursor = tail;
            if !name.starts_with("accept_f02_d_") {
                continue;
            }
