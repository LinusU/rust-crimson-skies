//! Evidence-report harness for task F02-C (`docs/contracts/CLI-EVIDENCE.md`,
//! schema `schemas/evidence.schema.json`).
//!
//! This test is deliberately **not** named `accept_f02_c_*`: it is not part of
...
//!    `docs/findings/evidence/F02-C.json`.
//!
...
//! shipped `cs-inspect` binary's own `inventory` output (the consumer
//! trace), `rustc --version` and `Cargo.lock`. Nothing is typed in by hand,
//! and the report describes the actual execution — a failing run produces a
//! failing report, which the validator rejects.
...
fn evidence_report_f02_c_writes_the_acceptance_report() {
    ...
    assert!(
        ...
        "no `accept_f02_c_` tests were recorded in {}",
        log_path.display()
    );
    ...
    // the spec's required capability for F02-C is exactly that.
    let retail_line = suite
        .assertions
        .iter()
        .find(|(name, _)| name.starts_with("accept_f02_c_retail_"))
        .unwrap_or_else(|| {
            panic!(
                "the retail acceptance test did not run: F02-C requires capability `retail`, \
    ...
    assert!(
        suite
            .assertions
            .iter()
            .any(|(name, _)| name.starts_with("accept_f02_c_")
                && !name.starts_with("accept_f02_c_retail_")),
    ...
    // The consumer trace: the shipped binary's own `inventory` output over
    // the original installation, written into the evidence directory.
    let report_path = evidence_dir.join("inventory-report.json");
    let output = Command::new(env!("CARGO_BIN_EXE_cs-inspect"))
        ...
        .output()
        .expect("the cs-inspect binary runs");
    assert!(
        output.status.success(),
        "cs-inspect inventory over $CS_GAME_DIR must succeed for the consumer trace: {}",
        String::from_utf8_lossy(&output.stderr)
    );
    let report = fs::read_to_string(&report_path).expect("the inventory report reads back");
    assert!(
        report.contains(&format!("\"install_sha256\": \"{install_sha256}\"")),
        "the consumer report must carry the same installation fingerprint the suite measured"
    );
    assert!(
    ...
    let report = format!(
        ...
        jstr(
            "acceptance suite run locally with the retail capability; this harness derives \
              ...
              cs-inspect inventory output, rustc and Cargo.lock; validated with \
...
/// root, so a path like `private/evidence/F02-C` written relative to the
...
/// `accept_f02_c_` tests from a recorded `cargo test` output.
fn parse_suite(log: &str) -> Suite {
    ...
    for line in log.lines() {
        ...
        while let Some(position) = cursor.find("test ") {
            ...
            cursor = tail;
            if !name.starts_with("accept_f02_c_") {
                continue;
            }
