//! Shared fixture helpers for the F02-C acceptance tests.
//!
...
impl TempTree {
    ...
    /// The tree's host root, ready to be handed to discovery or `--cs-path`.
    ...
    /// Creates a directory at `spelling` (a `/`-separated relative
    /// spelling) without putting any file in it.
    pub fn mkdir(&self, spelling: &str) {
        fs::create_dir_all(self.root.join(spelling)).expect("fixture directories are created");
    }
}
